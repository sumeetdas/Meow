...
module.exports = function (pServer, pRouter) {
    ...
    // send robots.txt
    pServer.get('/robots.txt', function (pRequest, pResponse) {
        pResponse.sendFile(utils.getFilePathRelativeToAppRoot('./robots.txt'));
    });
    ...
    // use user-defined router if available
    if (pRouter) {
        pServer.use(pRouter);
    }
